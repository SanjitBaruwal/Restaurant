import { Prism as SyntaxHighlighter } from "react-syntax-highlighter";
import { dracula } from "react-syntax-highlighter/dist/esm/styles/prism";
import ResuableHeader from "../../../components/ResuableHeader";
const Navs = () => {
  const Horizontal = `<ul class="nav flex-column">
   <li class="nav-item"><a class="nav-link active" href="#">Active</a></li>
   <li class="nav-item"><a class="nav-link" href="#">Link</a></li>
   <li class="nav-item"><a class="nav-link" href="#">Link</a></li>
   <li class="nav-item"><a class="nav-link disabled" href="#">Disabled</a></li>
 </ul>`;

  const pills = `<ul class="nav nav-pills">
   <li class="nav-item"><a class="nav-link active" href="#">Active</a></li>
   <li class="nav-item"><a class="nav-link" href="#">Link</a></li>
   <li class="nav-item"><a class="nav-link" href="#">Link</a></li>
   <li class="nav-item"><a class="nav-link disabled" href="#">Disabled</a></li>
 </ul>`;

  const fillAndJustify = `<ul class="nav nav-pills nav-fill">
   <li class="nav-item"><a class="nav-link active" href="#">Active</a></li>
   <li class="nav-item"><a class="nav-link" href="#">Longer nav link</a></li>
   <li class="nav-item"><a class="nav-link" href="#">Link</a></li>
 </ul>`;
  return (
    <div className="  bg-[#F9FAFD] py-[110px]   w-full  px-10 lg:px-[5%] xl:px-[12.5%] ">
      <div className="grid grid-cols-5 gap-4">
        <div className="col-span-5 lg:col-span-4 grid grid-cols-2  gap-4">
          <div className="col-span-2 ">
            <ResuableHeader
              title="Navs"
              desc="Documentation and examples of how to use Bootstrap’s included navigation components."
              goto="NAVS ON BOOTSTRAP"
              link="https://getbootstrap.com/docs/4.3/components/navs/"
            />
          </div>
          <div className="col-span-2  shadow-md h-fit flex flex-col ">
            <h2
              className="text-2xl bg-white p-4 text-black"
              style={{ fontFamily: "Merriweather" }}
            >
              Horizontal
            </h2>
            <div className=" h-fit flex flex-col gap-2 ">
              <div className="bg-[#F9FAFD] p-4  flex flex-col gap-2 ">
                <ul className="flex flex-wrap gap-3">
                  <li className="text-text">
                    <a href="#">Active</a>
                  </li>
                  <li className="text-text">
                    <a href="#">Link</a>
                  </li>
                  <li className="text-text">
                    <a href="#">Link</a>
                  </li>
                  <li>
                    <a
                      href="#"
                      className=" cursor-not-allowed "
                      onClick={(e) => e.preventDefault()}
                    >
                      Disabled
                    </a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
          <div className="col-span-2  shadow-md h-fit flex flex-col ">
            <h2
              className="text-2xl bg-white p-4 text-black"
              style={{ fontFamily: "Merriweather" }}
            >
              Vertical
            </h2>
            <div className=" h-fit flex flex-col gap-2 ">
              <div className="bg-[#F9FAFD] p-4  flex flex-col gap-2 ">
                <ul className="flex flex-col gap-3">
                  <li className="text-text">
                    <a href="#">Active</a>
                  </li>
                  <li className="text-text">
                    <a href="#">Link</a>
                  </li>
                  <li className="text-text">
                    <a href="#">Link</a>
                  </li>
                  <li>
                    <a
                      href="#"
                      className=" cursor-not-allowed "
                      onClick={(e) => e.preventDefault()}
                    >
                      Disabled
                    </a>
                  </li>
                </ul>
                <div className="select-text">
                  <SyntaxHighlighter language="html" style={dracula}>
                    {Horizontal}
                  </SyntaxHighlighter>
                </div>
              </div>
            </div>
          </div>

          <div className="col-span-2  shadow-md h-fit flex flex-col ">
            <h2
              className="text-2xl bg-white p-4 text-black"
              style={{ fontFamily: "Merriweather" }}
            >
              Pills
            </h2>
            <div className=" h-fit flex flex-col gap-2 ">
              <div className="bg-[#F9FAFD] p-4  flex flex-col gap-2 ">
                <ul className="flex flex-wrap items-center  gap-6">
                  <li className="bg-bgLightYellow text-white px-3 py-2 rounded-md">
                    <a href="#">Active</a>
                  </li>
                  <li className="text-text">
                    <a href="#">Link</a>
                  </li>
                  <li className="text-text">
                    <a href="#">Link</a>
                  </li>
                  <li>
                    <a
                      href="#"
                      className=" cursor-not-allowed "
                      onClick={(e) => e.preventDefault()}
                    >
                      Disabled
                    </a>
                  </li>
                </ul>
                <div className="select-text">
                  <SyntaxHighlighter language="html" style={dracula}>
                    {pills}
                  </SyntaxHighlighter>
                </div>
              </div>
            </div>
          </div>

          <div className="col-span-2  shadow-md h-fit flex flex-col ">
            <h2
              className="text-2xl bg-white p-4 text-black"
              style={{ fontFamily: "Merriweather" }}
            >
              Fill and Justify
            </h2>
            <div className=" h-fit flex flex-col gap-2 ">
              <div className="bg-[#F9FAFD] p-4  flex flex-col gap-2 ">
                <ul className="flex flex-wrap justify-between items-center  gap-6">
                  <li className="  bg-bgLightYellow text-white xs:px-4 sm:px-16 py-2 rounded-md">
                    <a href="#">Active</a>
                  </li>
                  <li className=" text-text">
                    <a href="#">Longer nav Link</a>
                  </li>
                  <li className=" text-text xs:px-4 sm:px-16">
                    <a href="#">Link</a>
                  </li>
                </ul>
                <div className="select-text ">
                  <SyntaxHighlighter language="html" style={dracula}>
                    {fillAndJustify}
                  </SyntaxHighlighter>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navs;
